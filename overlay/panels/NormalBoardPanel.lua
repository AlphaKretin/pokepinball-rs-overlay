-- Normal Ruby/Sapphire board overlay: the original (and still default)
-- overlay content -- spawn-pool grid below the screen, egg-hatch grid +
-- travel diagram + specials column in the right flange. Split out of
-- Overlay.lua -- see .claude/plans/partitioned-wiggling-papert.md.
--
-- Depends on globals set by Overlay.lua before this file is dofile'd:
...
-- AREA_ROULETTE_RUIN_SLOT (global, shared with FieldSelectPanel). The
-- Ruin-override branch never writes NextSlot/FarSlot, so they carry through
-- a Ruin visit unchanged and the travel after that resumes exactly where
-- the skipped normal travel would have left off.
...
-- gAreaRouletteTable, ADDR_AREA_ROULETTE_TABLE global), not from the
-- current area directly -- see main_board_travel_mode.c:150-153 (left picks
-- NextSlot, anything else picks FarSlot). Both fields are recomputed on
-- every spin/travel and hold valid resting values at all other times, so no
...
-- decomp source exactly. See docs/memory-map.md.
--
...
-- table-driven spawn) -- deliberately not read here.
...
-- self-extracted from ROM by GfxExtract.lua) are 48x32, drawn at native
-- size -- any resize call here is a real, lossy downscale done in software;
-- BizHawk's window zoom afterwards just magnifies whatever pixels we handed
-- it; it can't recover detail a software resize already threw away. So
-- native size + BizHawk's own (nearest-neighbor) window zoom is the only
-- combination that stays crisp.
NormalBoardPanel.PORTRAIT_W, NormalBoardPanel.PORTRAIT_H = 48, 32
local PORTRAIT_W, PORTRAIT_H = NormalBoardPanel.PORTRAIT_W, NormalBoardPanel.PORTRAIT_H
...
-- (see Draw.drawCdStack), not above/below it. Icons are the same 48x32
-- native size as the spawn-pool portraits, for the same no-lossy-resize
-- reason. Drawn at its own natural content width (see
-- TRAVEL_DIAGRAM_WIDTH below), not stretched to fill the flange.
...
-- fanned arrows between them to read clearly.
local TRAVEL_DIAGRAM_WIDTH = 2 * PORTRAIT_W + 57

-- Egg-hatch grid (right panel): field-wide (not area-scoped). Icons
-- self-extracted from ROM by GfxExtract.lua -- 24x24 native size (frame 0
-- of each species' hatch-animation sprite), same no-lossy-resize reasoning
-- as the portrait grid. Exactly 25 real per-field egg spawns (EGG_POOL_SIZE)
-- makes a clean 5x5 grid with no leftover slot -- Pichu (the table's 26th,
-- unused slot) is deliberately excluded.
...
-- column rather than a grid since there are only 4 entries.
...
-- aspect-ratio goal -- exposed for Overlay.lua's canvas joint-solve. Right
-- flange holds the egg-hatch grid (top-anchored) plus the dex-caught line
-- and the travel diagram (bottom-anchored, beside the spawn-pool grid which
-- sits below the screen at the same height).
--
-- The specials column doesn't need its own content-min term: it's
-- top-anchored right next to the egg grid (SPECIAL_COLUMN_X, based on the
-- egg grid's own width) and, confirmed live, already fits within the width
-- this term already reserves for the wider of the two existing terms (the
-- travel diagram) plus the ~37px of 16:9 ratio slack -- no extra width
-- needed. Similarly its own height (4 * SPECIAL_CELL_H = 137, + margins =
-- 145) is close to but doesn't exceed the egg grid's (143), and
-- minCanvasHeight is always governed by the below-screen region regardless.
NormalBoardPanel.CONTENT_MIN_RIGHT_PAD = math.max(EGG_GRID_COLUMNS * EGG_CELL_W + 8, TRAVEL_DIAGRAM_WIDTH + 8)
NormalBoardPanel.CONTENT_MIN_RIGHT_PANEL_HEIGHT = PANEL_TOP_MARGIN + EGG_GRID_ROWS * EGG_CELL_H + PANEL_BOTTOM_MARGIN
...
-- content-min term -- see drawEggAndSpecials.
...
-- below SCREEN_HEIGHT), so whichever needs more height governs.
NormalBoardPanel.CONTENT_MIN_BELOW_SCREEN_HEIGHT = math.max(CONTENT_MIN_SPAWN_HEIGHT, CONTENT_MIN_TRAVEL_HEIGHT)
...
function NormalBoardPanel.isEncounterRateUp()
...
function NormalBoardPanel.readEvolvablePartySet()
...
function NormalBoardPanel.readAreaCdProgress(area)
...
function NormalBoardPanel.readEggPool(field, queueSet)
	local pool = {}
...
-- ROM-data row.
function NormalBoardPanel.readSpecials(field)
...
local function drawPortraitCell(x, y, entry)
	...
	Draw.drawPortraitBorder(x, y, PORTRAIT_W, PORTRAIT_H, Draw.borderColorFor(entry))

	local exclusiveColor = nil
	if entry.exclusive == "2" then
		exclusiveColor = Draw.TWO_EXCLUSIVE_MARKER_COLOR
	elseif entry.exclusive == "3" then
		exclusiveColor = Draw.THREE_EXCLUSIVE_MARKER_COLOR
	end
	...
	if entry.rare and exclusiveColor then
		...
		Draw.drawMarker(x + PORTRAIT_W - Draw.MARKER_SIZE + 2, y - 2, exclusiveColor)
	...
	elseif exclusiveColor then
		Draw.drawMarker(x - 2, y - 2, exclusiveColor)
	end
end
...
local function drawTravelDiagram(x, y, width, areaIndex, areaCdCount, areaTotal,
	...
	Draw.safeDrawImage(NormalBoardPanel.areaIconPath(leftAreaIndex), leftX, topIconY, PORTRAIT_W, PORTRAIT_H)
	Draw.safeDrawImage(NormalBoardPanel.areaIconPath(rightAreaIndex), rightX, topIconY, PORTRAIT_W, PORTRAIT_H)
	Draw.safeDrawImage(NormalBoardPanel.areaIconPath(areaIndex), centerX, bottomIconY, PORTRAIT_W, PORTRAIT_H)

	Draw.drawCdStack(leftX, topIconY, PORTRAIT_W, PORTRAIT_H, true, leftCdCount, leftTotal)
	Draw.drawCdStack(rightX, topIconY, PORTRAIT_W, PORTRAIT_H, false, rightCdCount, rightTotal)
	Draw.drawCdStack(centerX, bottomIconY, PORTRAIT_W, PORTRAIT_H, true, areaCdCount, areaTotal)
	...
	-- computation -- see Draw.drawArrow.
	local axisX = Draw.roundPx(centerX + PORTRAIT_W / 2)
	local startY = Draw.roundPx(bottomIconY)
	local endY = Draw.roundPx(topIconY + PORTRAIT_H)
	local rightStartX = Draw.roundPx(centerX + PORTRAIT_W - ARROW_START_INSET)
	local rightEndX = Draw.roundPx(rightX + PORTRAIT_W / 2)

	Draw.drawArrow(rightStartX, startY, rightEndX, endY, "white")
	Draw.drawArrow(2 * axisX - rightStartX, startY, 2 * axisX - rightEndX, endY, "white")
end
...
local function drawEggHatchCell(x, y, entry)
	Draw.safeDrawImage(eggIconPath(entry.name), x, y, EGG_ICON_W, EGG_ICON_H)
	Draw.drawPortraitBorder(x, y, EGG_ICON_W, EGG_ICON_H, Draw.borderColorFor(entry))
end

-- Egg-hatch grid (top-anchored) + dex-caught line + specials column --
-- shared by NormalBoardPanel.draw() and FieldSelectPanel, which wants the
-- same content but not the travel diagram (no current area to show one
-- for). Does not draw the panel background rectangle or the travel diagram
-- -- callers own those.
function NormalBoardPanel.drawEggAndSpecials(pool, specials, caught, rateUp)
...
function NormalBoardPanel.draw()
	...
	local rateUp = NormalBoardPanel.isEncounterRateUp()
	local areaCdCount, areaTotal = NormalBoardPanel.readAreaCdProgress(areaIndex)
	local caught = NormalBoardPanel.readDexCaughtCount()

	local leftAreaIndex, rightAreaIndex = readTravelOptions()
	local leftCdCount, leftTotal = NormalBoardPanel.readAreaCdProgress(leftAreaIndex)
	local rightCdCount, rightTotal = NormalBoardPanel.readAreaCdProgress(rightAreaIndex)

	-- Right panel: field-wide egg-hatch pool (top-anchored) -- unlike the
	-- spawn-pool grid this never changes with travel, only with a
	-- Ruby/Sapphire field switch -- plus the dex-caught line, specials
	-- column, and the travel diagram bottom-anchored beside the spawn-pool
	-- grid (which sits below the screen, see drawSpawnPanel).
	local panelHeight = SCREEN_HEIGHT + DOWN_PAD
	Draw.drawPanelBackground(GAME_X + SCREEN_WIDTH, 0, RIGHT_PAD, panelHeight)

	NormalBoardPanel.drawEggAndSpecials(eggPool, specials, caught, rateUp)

	local diagramX, diagramY = GAME_X + SCREEN_WIDTH + 4, GAME_Y + SCREEN_HEIGHT + PANEL_TOP_MARGIN
	drawTravelDiagram(diagramX, diagramY, TRAVEL_DIAGRAM_WIDTH, areaIndex, areaCdCount, areaTotal,
		leftAreaIndex, leftCdCount, leftTotal, rightAreaIndex, rightCdCount, rightTotal)
